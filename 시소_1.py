...
mock_data = {
    "journeys": [
        ...
        {"id": 3, "type": "클래스", "title": "[우리 동네] 에세이 한 문장 글쓰기", "price": 29000, "region": "서울", "description": "방배동 북카페에서 에세이 작가와 함께하는 글쓰기 클래스입니다.", "schedule": ["매주 수요일 오후 2시 (총 4회)"], "tags": ["#글쓰기", "#정적활동"]},
        ...
        {"id": 5, "type": "클래스", "title": "[우리 동네] 주말 요가 클래스", "price": 120000, "region": "서울", "description": "주말 아침, 요가와 명상으로 몸과 마음의 균형을 되찾는 시간입니다.", "schedule": ["매주 토요일 오전 10시 (총 4회)"], "tags": ["#요가", "#운동", "#정적활동"]},
        ...
        {"id": 8, "type": "여행", "title": "기장 대게 한상, 바닷가를 품다", "price": 349000, "region": "부산", "description": "기장의 명물, 신선한 대게를 푸짐한 한상차림으로 즐기는 스페셜 미식 여행.", "schedule": ["당일 또는 1박 2일", "대게 코스 요리 포함"], "tags": ["#부산", "#기장", "#대게", "#바다", "#맛집"]},
        ...
        {"id": 12, "type": "여행", "title": "전주 한상 가득 미식기행", "price": 289000, "region": "경남", "description": "맛의 고장 전주에서 푸짐하게 차려진 전통 한정식을 맛보는 미식 기행입니다.", "schedule": ["1박 2일 프로그램", "한정식 코스 포함"], "tags": ["#전주", "#미식", "#한정식", "#맛집", "#밥상"]},
        {"id": 13, "type": "여행", "title": "광주 오미의 맛, 정겨운 밥상", "price": 309000, "region": "경남", "description": "광주의 다섯 가지 맛을 대표하는 음식들을 맛보며 남도의 정을 느끼는 밥상 투어.", "schedule": ["1박 2일 프로그램", "광주 5미 체험"], "tags": ["#광주", "#밥상", "#오미", "#맛집"]},
        {"id": 14, "type": "여행", "title": "춘천 닭갈비와 낭만의 시장길", "price": 299000, "region": "강원", "description": "춘천의 명물 닭갈비를 맛보고, 낭만이 가득한 중앙시장을 거닐어보는 여행입니다.", "schedule": ["당일 또는 1박 2일", "닭갈비 식사 포함"], "tags": ["#춘천", "#닭갈비", "#시장", "#낭만"]},
        {"id": 15, "type": "여행", "title": "속초 바다 먹거리 산책", "price": 299000, "region": "강원", "description": "속초 중앙시장에서 신선한 해산물과 닭강정 등 다양한 먹거리를 즐기는 산책 코스.", "schedule": ["당일 또는 1박 2일", "시장 투어 포함"], "tags": ["#속초", "#바다", "#먹거리", "#산책", "#시장"]},
        ...
        {"id": 18, "type": "여행", "title": "느린 숲길, 정선 힐링여행 2박 3일", "price": 309000, "region": "강원", "description": "강원도 정선의 고요한 숲길을 천천히 걸으며 자연 속에서 온전한 쉼을 얻는 여행.", "schedule": ["2박 3일 프로그램", "트레킹 포함"], "tags": ["#정선", "#숲길", "#힐링", "#여행", "#걷기많음"]},
        {"id": 19, "type": "여행", "title": "소리꾼 치유받는 남해 명상여행", "price": 329000, "region": "전남", "description": "남해의 아름다운 자연 속에서 명상과 소리 치유 프로그램을 통해 마음을 돌봅니다.", "schedule": ["1박 2일 프로그램", "명상 및 힐링 포함"], "tags": ["#남해", "#명상", "#여행", "#치유", "#힐링"]},
        {"id": 20, "type": "여행", "title": "고요한 하루, 강화도 템플스테이", "price": 259000, "region": "경기", "description": "복잡한 일상에서 벗어나 강화도의 사찰에서 고요한 하루를 보내는 템플스테이입니다.", "schedule": ["1박 2일 템플스테이 프로그램 참여"], "tags": ["#강화도", "#템플스테이", "#고요", "#힐링", "#정적활동"]}
    ]
}
...
async def get_my_page_info(token: Optional[str] = Header(None, alias="Authorization")):
    if not token or not token.startswith("Bearer "): raise HTTPException(status_code=401, detail="인증 헤더가 필요합니다.")
    actual_token = token.split(" ")[1]
    if actual_token != "fake-jwt-token-for-jeongyoungho": raise HTTPException(status_code=401, detail="인증되지 않은 사용자입니다.")
    saved_items_details = [item for item in mock_data["journeys"] if item["id"] in mock_user_data["saved_item_ids"]]
    my_page_data = { "profile": {"username": mock_user_data["username"], "profile_image_url": mock_user_data["profile_image_url"], "catchphrase": mock_user_data["catchphrase"]}, "recent_searches": mock_user_data["recent_searches"], "saved_items": saved_items_details }
    return my_page_data
